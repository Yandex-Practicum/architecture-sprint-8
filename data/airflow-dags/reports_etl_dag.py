...
dag = DAG(
    ...
    schedule_interval='*/3 * * * *',  # Тестовый интервал: каждые 3 минуты (для отладки)
    catchup=False,
    tags=['reports', 'etl', 'bionicpro'],
)


def convert_execution_date(execution_date):
    """
    Преобразует execution_date из Airflow context в обычный Python datetime
    для совместимости с psycopg2 и другими библиотеками
    """
    if hasattr(execution_date, 'naive'):
        # Pendulum datetime (Airflow 2.x) - преобразуем в naive datetime
        return execution_date.naive()
    elif isinstance(execution_date, datetime):
        # Уже обычный datetime
        return execution_date
    else:
        # Преобразуем через строку
        date_str = str(execution_date).replace('Z', '+00:00').split('+')[0]
...
def extract_sensor_data(**context):
    ...
    """
    execution_date = convert_execution_date(context['execution_date'])
    ...
    # Получаем данные из PostgreSQL и преобразуем в DataFrame
    records = postgres_hook.get_records(query, parameters=[start_date, end_date])
    
    # Создаем DataFrame из полученных записей
    if records:
        df = pd.DataFrame(
            records,
            columns=[
                'user_id', 'device_id', 'timestamp', 'sensor_type',
                'sensor_value', 'movement_type', 'movement_count', 'usage_duration_seconds'
            ]
        )
    else:
        # Если данных нет, создаем пустой DataFrame с правильными колонками
