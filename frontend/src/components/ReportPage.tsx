// ReportPage.tsx — обновлён для работы через bionicpro-auth (BFF)
//
// Что изменилось vs оригинал:
//  - Убран keycloak.token из запроса (фронтенд токенов не видит)
//  - Запрос идёт на /api/reports (bionicpro-auth), а не напрямую на бэкенд
//  - Сессия передаётся через cookie (credentials: 'include')
//  - Логин через редирект на /auth/login (PKCE flow инициирует BFF)
...
const ReportPage: React.FC = () => {
  ...
  // Проверяем, есть ли активная сессия (cookie уже установлен BFF)
  useEffect(() => {
    fetch(`${AUTH_URL}/auth/userinfo`, {
      credentials: 'include',   // ключевое: отправляем HttpOnly cookie
    })
  ...
  const handleLogin = () => {
    // Перенаправляем на BFF — он инициирует PKCE flow с Keycloak
  ...
  const downloadReport = async () => {
    try {
      ...
      // Запрос через BFF: cookie отправляется автоматически
      // user_id не передаём — BFF берёт его из токена (RBAC)
