...
title Диаграмма контейнеров BionicPRO
...
Person(mlEngineer, "ML-инженер", "")

System_Boundary(bionicpro, "BionicPRO") {
    Container(webshop, "Интернет-магазин", "Битрикс24", "")
    Container(crm, "CRM", "Битрикс24", "")
    ContainerDb(crmDb, "CRM DB", "Oracle Database 12", "")
    Container(api, "API", "Spring Boot / ElasticSearch / etc.", "")
    ContainerDb(mainDb, "База данных", "PostgreSQL", "")
}

Container(chipProgram, "Программа в чипе протеза", "Embedded software", "Чтение данных с датчиков, распознавание движений и их воспроизводство. Чтение команд из приложения и их воспроизводство, передача телеметрии")
Container(tuningApp, "Приложение для донастройки протеза", "iOS, Android", "")
Container(cliTool, "cli tool", "Spring Boot / ElasticSearch / etc.", "")
...
Rel(tuningApp, chipProgram, "Управление протезом")
Rel(chipProgram, api, "Отправка технической телеметрии")
Rel(tuningApp, api, "")

Rel(api, mainDb, "")
Rel(cliTool, mainDb, "")
Rel(mlEngineer, cliTool, "")
...
Для подготовки архитектуры решения используйте С4-модель и схемы в plantuml.
...
1.  Используя механизм Identity Brokering, реализуйте аутентификацию пользователей через внешний Identity Provider Яндекс ID. Обратите внимание, что сервис протезов получает данные профиля пользователя из Яндекса.
...
Для подготовки архитектуры решения используйте C4 в plantuml
...
1.  Диаграмма архитектуры системы в модели C4 в plantuml.
...
1.  Диаграмма архитектуры системы C4 в plantuml.
